...
export default function Devices(){
  ...
  async function createDevice(dev_name, dev_ip, dev_port, dev_status, dev_url,  dev_part){
  ...
  const {values, isSubmiting, setFieldValue, handleSubmit, errors} = useFormik({
    ...
    onSubmit: values =>{
      ...
      const {name, ip, port, device, status, url} = values;
      const response = createDevice(name, ip,  port, status, url, device);
    ...
    >    
      ...
      <View style={styles.formHeader}>
        ...
        <View style={styles.formControl}>
              <TextInput 
                  placeholder="Porta"
                  onChangeText={text => setFieldValue('port', text)}
              />
        </View>
        <View style={styles.formControl}>
            <Picker
